...
class Transcriber:
    ...
    def _auto_detect_microphone(self) -> int:
        ...
        try:
            ...
            if device_info["max_input_channels"] > 0:
                logger.info(
                    f"Auto-detected microphone: [{default_idx}] {device_info['name']}"
                )
                return default_idx
        except Exception as e:
    ...
    def _stop_recording(self) -> None:
        ...
        if recording_duration < self.audio_config.min_utterance_duration:
            if self.verbose:
                logger.debug(
                    f"Recording too short ({recording_duration:.2f}s), discarding"
                )
            self._frames = []
            return
        ...
        if self.verbose:
            logger.info(
                f"Recording stopped ({recording_duration:.2f}s), transcribing..."
            )
